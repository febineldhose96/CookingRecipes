...
class Recipe {
  ...
  async getSingleRecipe(id) {
    var sql = "select * from recipe_details where recipe_details.recipe_id = ?";
    // As we are not inside an async function we cannot use await
    // So we use .then syntax to ensure that we wait until the
    // promise returned by the async function is resolved before we proceed
    var recipe_details = await db.query(sql, [id]).then((res) => res);
    return recipe_details;
  }
  async searchRecipe(word) {
    var sql =
      "SELECT * FROM recipe_details WHERE recipe_details.recipe_name LIKE ?";
    // As we are not inside an async function we cannot use await
    // So we use .then syntax to ensure that we wait until the
    // promise returned by the async function is resolved before we proceed
    var recipe_details = await db.query(sql, [`${word}%`]).then((res) => res);
